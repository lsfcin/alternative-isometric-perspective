/*
  To-Do:
    ...
    token: desabilitar para somente o próprio token (low priority)
...
// Patch para o TileHUD
class IsometricTileHUD {
  static init() {
    libWrapper.register(
      'isometric-module',
      'TileHUD.prototype.setPosition',
      IsometricTileHUD._setPosition,
      'OVERRIDE'
    );
  }

  static _setPosition(function_) {
    if (!this.object) return;

    // Obter a posição do tile no canvas
    const tile = this.object;
    const transform = tile.transform;
    
    // Calcular o centro do tile considerando a perspectiva isométrica
    const center = this._getIsometricCenter(tile);
    
    // Ajustar posição do HUD
    const position = {
      x: center.x - (this.element.width() / 2),
      y: center.y - this.element.height() - 10
    };
  ...
  static _getIsometricCenter(tile) {
    // Obter as dimensões do tile
    const w = tile.document.width;
    const h = tile.document.height;
    
    // Obter a posição atual do tile
    const position = tile.document.position;
    
    // Calcular o centro do tile na perspectiva isométrica
    const isometricTransform = CONFIG.Canvas.layers.tiles.gridPerspective;
    
    if (!isometricTransform) {
      // Fallback para cálculo normal se não estiver em modo isométrico
      return {
        x: position.x + (w / 2),
        y: position.y + (h / 2)
      };
    ...
    // Converter coordenadas do tile para coordenadas isométricas
    const isoX = position.x + (w / 2);
    const isoY = position.y + (h / 2);

    // Aplicar transformação isométrica
    const angle = Math.PI / 4; // 45 graus em radianos
    const isoScale = Math.cos(angle);

    const transformedX = (isoX - isoY) * isoScale;
    const transformedY = ((isoX + isoY) * isoScale) / 2;

    // Ajustar com base na elevação do tile (se aplicável)
    const elevation = tile.document.elevation || 0;
    const gridSize = canvas.grid.size;
    const gridScale = canvas.grid.grid.options.dimensions.distance;
    const elevationOffset = (elevation / gridScale) * gridSize;

    return {
      x: transformedX + canvas.stage.pivot.x,
      y: transformedY + canvas.stage.pivot.y - elevationOffset
    };
  }
}

// Registrar o patch durante a inicialização do módulo
Hooks.once('init', () => {
  IsometricTileHUD.init();
});

// Patch adicional para garantir que o HUD seja atualizado quando o tile for movido
Hooks.on('updateTile', (tile, changes) => {
  if ('x' in changes || 'y' in changes || 'elevation' in changes) {
    const hud = canvas.hud.tile;
    if (hud && hud.object === tile) {
      hud.setPosition();
    }
  }
});
...
export const TileHUDPatch = IsometricTileHUD;
*/